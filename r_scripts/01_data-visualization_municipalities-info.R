...
theme_update(
  ...
  plot.title = element_textbox_simple(size = 20, face = 'bold', margin = margin(5,0,5,0), halign = 0),
  plot.subtitle = element_textbox_simple(size = 12, margin = margin(0,0,25,0)),
  plot.caption = element_textbox_simple(size = 7, halign = 0, margin = margin(13,0,0,0)),
)
...
p1 <- df %>% filter(cluster != 0) %>%
  ...
  annotate(geom = "text", x = 3, y = 1400,
           size = 3.45, family = 'Optima', fontface = 'italic',
           label = "1304 ha affected\nby 1 wildfire") +
  annotate(geom = "text", x = 12, y = 1000,
           size = 3.45, family = 'Optima', fontface = 'italic',
  ...
  scale_x_continuous(breaks = seq(0,13,2), labels = paste(seq(0,13,2), "wildfires")) + 
...
p2 <- df %>% filter(cluster != 0) %>%
  ...
  labs(
    ...
    caption = "**Source:** _Comisión Nacional Forestal (CONAFOR), 2017_.<br>**Visualization by Isaac Arroyo (@unisaacarroyov on Twitter and Behance)**"
  ) +
  coord_sf(xlim = c(3700663, 3994857),
           ylim = c(897964.9, 1140000)) + 
  theme(
    plot.subtitle = element_textbox_simple(margin = margin(-20,0,20,0)),
  )
p2
ggsave("./images/01_municipalities-info_map.png", units = "px", height = 675, width = 540, scale = 3)

(p1/p2) + plot_layout(heights = c(0.5,0.5))
ggsave("./images/01_municipalities-info.png", units = "px", height = 1350, width = 540, scale = 3)
